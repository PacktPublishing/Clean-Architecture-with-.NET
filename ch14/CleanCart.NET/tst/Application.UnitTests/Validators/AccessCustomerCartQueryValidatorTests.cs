...
public class AccessCustomerCartQueryValidatorTests
{
    private readonly AccessCustomerCartQueryValidator _validator = new();

    [Fact]
    public void Should_Have_Error_When_AuthorizationUserId_Is_Empty()
    {
        var model = new AccessCustomerCartQuery(Guid.Empty, Guid.NewGuid());
    ...
    {
        var model = new AccessCustomerCartQuery(Guid.NewGuid(), Guid.Empty);
        var result = _validator.TestValidate(model);
    ...
    {
        var model = new AccessCustomerCartQuery(Guid.NewGuid(), Guid.NewGuid());
        var result = _validator.TestValidate(model);
