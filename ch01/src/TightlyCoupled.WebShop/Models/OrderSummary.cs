...
public class OrderSummary
{
    ...
    // Business logic in defaultItem - bad practice
    public string GetFormattedTotal() => TotalAmount.ToString("C");
    ...
    public bool IsRecentOrder() => (DateTime.Now - OrderDate).TotalDays <= 30;

    // Data access in defaultItem - terrible practice
    ...
    // More data access in defaultItem
    ...
    // External service call in defaultItem
    public string GetTrackingInfo()
    {
        if (Status is not "S" and not "D")
        {
        ...
        try
        {
            ...
            HttpResponseMessage response = httpClient.PostAsync("http://shipping.company.com/api/track", content).Result;
    ...
    // File system dependency in defaultItem
    public void SaveOrderSummaryToFile()
    {
        try
        {
            ...
            var json = JsonSerializer.Serialize(summary, jsonSerializerOptions);
    ...
    // Email functionality in defaultItem - mixing concerns
