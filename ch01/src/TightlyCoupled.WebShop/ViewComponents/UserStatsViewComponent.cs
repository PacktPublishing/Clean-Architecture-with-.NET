...
/// </summary>
public class UserStatsViewComponent(ApplicationDbContext context) : ViewComponent
{
    static readonly string[] _defaultItem = ["Default Item"];
    ...
    public IViewComponentResult Invoke()
    {
        try
        {
            ...
            try
            {
                ...
                HttpResponseMessage response = httpClient.PostAsync("http://recommendations.company.com/api/get-recommendations", content).Result;
            ...
            {
                stats.Recommendations = ["Featured Item 1", "Featured Item 2"]; // Fallback
            }
        ...
        catch (Exception ex)
        {
            ...
            return View(new UserStatsViewModel
            {
                ...
                UserTier = "Standard",
                Greeting = "Welcome!",
                Recommendations = _defaultItem
            });
        }
    }

    // File system dependency for user personalization
    static string GetPersonalizedGreeting(string userEmail)
    {
        try
        {
