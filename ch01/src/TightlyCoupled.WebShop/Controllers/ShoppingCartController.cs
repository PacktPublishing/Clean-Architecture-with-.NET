...
public class ShoppingCartController : Controller
{
    ...
    readonly ILogger<ShoppingCartController> _logger;

    // Hard-coded configuration - TERRIBLE practice for demonstrating tight coupling
    const string LOG_FILE_PATH = @"C:\Logs\WebShop\cart_operations.log";
    const string INVENTORY_FILE_PATH = @"C:\Data\inventory.xml";
    const string CONFIG_FILE_PATH = @"C:\Config\webshop_config.ini";
    const string TAX_SERVICE_URL = "http://taxservice.example.com";
    const string ANALYTICS_URL = "http://analytics.company.com/api/events";
    const string SMTP_SERVER = "smtp.company.com";
    const int SMTP_PORT = 587;
    const string EMAIL_USERNAME = "webshop@company.com";
    const string EMAIL_PASSWORD = "hardcoded_password_123";
    const string DB_CONNECTION = "Server=(localdb)\\mssqllocaldb;Database=TightlyCoupledWebShop;Trusted_Connection=true;MultipleActiveResultSets=true";
    ...
    static bool ValidateBusinessRules(string itemName, int quantity, decimal price)
    {
        ...
        if (itemName.Contains("premium", StringComparison.OrdinalIgnoreCase) && DateTime.Now.Hour < 9)
        {
    ...
    static void UpdateInventoryFile(string itemName, int quantityChange)
    {
        try
        {
            ...
            XmlNode? itemNode = doc.SelectSingleNode($"//item[@name='{itemName}']");
    ...
    void SendAnalyticsData(string userEmail, string action, string itemName, int quantity, decimal price)
    {
        try
        {
            ...
            HttpResponseMessage response = httpClient.PostAsync(ANALYTICS_URL, content).Result;
    ...
    static void NotifyWarehouseHighValueItem(string userEmail, string itemName, int quantity, decimal price)
    {
        try
        {
    ...
    static void SendErrorNotificationToWarehouse(string userEmail, string errorMessage)
    {
        try
        {
            var configLines = System.IO.File.ReadAllLines(CONFIG_FILE_PATH);
    ...
    static void SendManagerApprovalEmail(string userEmail, decimal orderAmount, string approvalFilePath)
    {
        try
        {
            var message = new MimeMessage();
...
WebShop System"
    ...
    public async Task<IActionResult> RemoveItem(string name)
    {
        ...
        CartItem? itemToRemove = await _context.CartItems
            .FirstOrDefaultAsync(c => c.UserId == userId && string.Equals(c.ItemName, name, StringComparison.OrdinalIgnoreCase));
    ...
    public IActionResult Checkout([FromBody] CartDetails details)
    {
        try
        {
            ...
            if (requiresManagerApproval && subtotal > 2000)
            {
                ...
                System.IO.File.WriteAllText(approvalFile, JsonSerializer.Serialize(approvalRequest, jsonSerializerOptions));
            ...
            for (var attempt = 1; attempt <= 3; attempt++)
            {
                try
                {
                    ...
                    HttpResponseMessage response = httpClient.PostAsync($"{TAX_SERVICE_URL}/calculateTax", content).Result;

                    if (response.IsSuccessStatusCode)
                    {
                        var taxResponse = response.Content.ReadAsStringAsync().Result;
                        dynamic? taxData = JsonSerializer.Deserialize<dynamic>(taxResponse);
            ...
            try
            {
                ...
                HttpResponseMessage shippingResponse = shippingClient.PostAsync("http://shipping.company.com/api/calculate", shippingContent).Result;

                if (shippingResponse.IsSuccessStatusCode)
                {
                    dynamic? shippingData = JsonSerializer.Deserialize<dynamic>(shippingResponse.Content.ReadAsStringAsync().Result);
            ...
            var paymentResult = "";
            var paymentMethod = totalPrice > 1000 ? "CreditCard" : "DebitCard";

            for (var paymentAttempt = 1; paymentAttempt <= 2; paymentAttempt++)
            {
                try
                {
...
WebShop Team";
                ...
                System.IO.File.WriteAllText(failureFile, JsonSerializer.Serialize(failureData, jsonSerializerOptions));
            ...
            try
            {
                ...
                HttpResponseMessage warehouseResponse = warehouseClient.PostAsync("http://warehouse.company.com/api/fulfill", warehouseContent).Result;
            ...
            try
            {
                ...
                HttpResponseMessage crmResponse = crmClient.PostAsync("http://crm.company.com/api/update-customer", crmContent).Result;
            ...
            System.IO.File.WriteAllText(orderSummaryFile, JsonSerializer.Serialize(orderSummary, jsonSerializerOptions));
...
";
            ...
            // Create and store confirmation view defaultItem
            var confirmationViewModel = new OrderConfirmationViewModel
            {
                ...
                Items = cartItems.ConvertAll(i => new CartItemViewModel
                {
                    ItemName = i.ItemName,
                    Price = i.Price,
                    Quantity = i.Quantity
    ...
    public IActionResult OrderConfirmation()
    {
        ...
        {
            OrderConfirmationViewModel? model = JsonSerializer.Deserialize<OrderConfirmationViewModel>(json);
            return View(model);
        }
